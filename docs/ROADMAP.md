...
### 2026-08-07 — signed, notarized, and accepted by Gatekeeper

The first fully clean build. `spctl` returns `accepted / source=Notarized
Developer ID`; `stapler validate` passes, so it works offline as well as on.

Five things went wrong on the way, none of them the certificate, and each cost
real time because the error named the wrong thing:

1. **`macos-14` ships Swift 5.10** and the sidecar's manifest declares tools 6.0,
   so the release died before it reached signing. Fixed by moving to `macos-15`,
   plus an explicit Swift assertion — SwiftPM's message blames the package when
   the runner is what's behind.
2. **A locked keychain hung CI for four hours.** A new keychain re-locks after
   five minutes idle, and the build spends longer than that compiling Rust before
   it reaches `codesign`. A locked keychain does not fail — it blocks on an
   interactive prompt that cannot exist on a runner. Fixed with
   `security set-keychain-settings`, a Rust cache to shorten the window, and
   `timeout-minutes` so the next hang costs an hour rather than six. That single
   hang burned roughly 2,400 billable minutes and exhausted the Actions
   allowance, which is why the release was finished locally.
3. **An empty environment variable is not an absent one.** `.env` used `""` to
   mean "unused", `set -a` exported it as *set*, and Tauri branches on presence:
   it tried to import a zero-byte certificate, then to notarize with no
   credentials. Both errors blamed the credential rather than the emptiness.
4. **An Apple ID password is not an app-specific password.** Apple rejects the
   account password outright; app-specific ones are generated by Apple and look
   like `xxxx-xxxx-xxxx-xxxx`.
5. **Notarization took about 20 hours**, against a documented 2–15 minutes, with
   Apple's status page reporting the notary as operational throughout. There is
   no queue depth, position, or ETA published — `notarytool` can only report your
   own submissions. Any timeout around notarization has to assume this is
   possible.

**Not yet proven:** that a DMG installs cleanly on a *second* Mac, and that the
updater actually updates. Both need two published releases and hardware this
...
**Status:** ✅ **Signing and notarization are done and proven** (2026-08-07). The
Developer ID certificate exists, the app is signed, Apple has notarized it, the
ticket is stapled, and the tool Gatekeeper itself consults agrees:
...
Full chain: `Developer ID Application: Wenlong Chen (MAK855QGSV)` →
`Developer ID Certification Authority` → `Apple Root CA`, verified `--deep
--strict` so the nested Swift sidecar is covered too.

**What remains of the done-when:** "installs on a clean machine with no Gatekeeper
warning **and successfully auto-updates from the previous version**." The first
half is now evidenced. The second needs two published releases and a second Mac,
so it stays open — and it is the half that actually matters, because the updater
has never been watched doing its job.
